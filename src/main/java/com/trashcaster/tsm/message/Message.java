	...
	public static class ServerHandler implements IMessageHandler<Message, IMessage> {
		...
		public IMessage onMessage(final Message message, final MessageContext ctx) {
			...
			mainThread.addScheduledTask(new Runnable() {
				@Override
				public void run() {
					System.out.println(String.format("Received %s from %s", message.string, ctx.getServerHandler().playerEntity.getName()));
					if (message.string.equals("accessoryinv")) {
					    ...
					    ctx.getServerHandler().playerEntity.openGui(TSM.INSTANCE, 0, ctx.getServerHandler().playerEntity.worldObj, x, y, z);
					}
				}
			});
